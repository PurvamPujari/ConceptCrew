...
public class Main3Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        stringArrayAdapter = new ArrayAdapter<>(this,R.layout.fak, stringArrayList);
        stringArrayAdapter1 = new ArrayAdapter<>(this,R.layout.fak, stringArrayList);
    ...
    public void me(View view) {
        ...
        String getInput = " ME    : " +ques;
        stringArrayList.add(getInput);
        stringArrayAdapter.notifyDataSetChanged();
        if(ques.toLowerCase().contains("codesh")){
            if(ques.toLowerCase().contains("what")){
                reply+="CodeShastra event intends to provide a platform for students from all over Mumbai , where they can learn certain invaluable skills about product development by building projects from scratch and competing with fellow students and in the process, can help develop various applications, websites that would be of utility for the society.";
            c=1;}
                if(ques.toLowerCase().contains("when")){
                reply+="\n CodeShastra event was organised on 23rd and 24th January 2016 .";
            c=1;}
            if(ques.toLowerCase().contains("where")){
                reply+="\n CodeShastra event was held in D.J. Sanghvi College Of Engineering .";
            c=1;}
            if(ques.toLowerCase().contains("team")||ques.toLowerCase().contains("students")) {
                reply += "\n Teams in codeshastra comprises of three or four students together .\nThe teams developing these feasible solutions comprised of students from the IT, Computers and EXTC departments as well as students from other colleges. .";
                c=1;}
            if(ques.toLowerCase().contains("web")||ques.toLowerCase().contains("app")||ques.toLowerCase().contains("solution")||ques.toLowerCase().contains("problem")){
                reply+="\n Codeshastra comprises of feasible solutions to the problems in the form of applications or websites .";
        c=1;}
            if(ques.toLowerCase().contains("organi")){
                reply+="\n Codeshastra was organized in association with CSI Mumbai .";
                c=1;}
            if(ques.toLowerCase().contains("theme")||ques.toLowerCase().contains("topic")){
                reply+="\n The theme of the event was “Swachh Bharat” .";
                c=1;}
            if(ques.toLowerCase().contains("motiv")){
                reply="\n With this Tech no-Social event Codeshastra, the students not only get exposed to the concept of a hackathon, but also invent their own confection at the event.";
                c=1;}
            if(c==0){
                reply+="\n Codeshastra was the flagship event of DJCSI and was held on 23rd and 24th Jan within the college premises. There were 130 participants present for the 2-day event. The theme of the event was “Swachh Bharat” and was organized in association with CSI Mumbai.";
            }
        }
        else
        reply=elzTalk(ques);
